...
`fast.adonis`<-
           ...
           )
  {
    ...
    if(!is.null(order_list) & by%in%"terms"){
    ...
    temp.dim <- ifelse(is.null(num_orders)|is.null(order_list),nterms*2,c(nterms*2+length(list.i)-num_orders))
    ...
    if ( !is.null( boot.times) ){
      ...
      if ( isParal && isMulticore ){
        boot.R2.set <- t(matrix(unlist(mclapply(1:boot.times,function(ind_boot){
          ...
          mc.cores = parallel)),nrow=temp.dim ,ncol=boot.times))
      }else {
    ...
    if ( permutations>0 & all(weights==1) ){
      ...
      if ( isParal && isMulticore ){
        R2_set_perm <- t(matrix(unlist(mclapply(1:permutations,function(ind_perm){
          ...
          mc.cores = parallel)),nrow=temp.dim+2,ncol=boot.times))
    ...
    if ( by=="terms" & nterms>=2 & (is.null(num_orders)) ){
      ...
      F.Mod <- SumsOfSqs[-c(length(SumsOfSqs)-1,length(SumsOfSqs))]/(df.Exp)/(SumsOfSqs[c(length(SumsOfSqs)-1)]/df.Res)
      P <- (rowSums(t(F.Mod.perm[,c(2,(1+nterms+1:(nterms-1)))])>=as.vector(F.Mod))+1)/(permutations+1)
    ...
    if ( by=="margin" & nterms>=2 ){
      ...
      F.Mod <- SumsOfSqs[-c(length(SumsOfSqs)-1,length(SumsOfSqs))]/(df.Exp)/(SumsOfSqs[c(length(SumsOfSqs)-1)]/df.Res)
      P <- (rowSums(t(F.Mod.perm[,c(2:(1+nterms))])>=as.vector(F.Mod))+1)/(permutations+1)
      tab <- data.frame(Df = c(df.Exp, df.Res, n-1),
                        SumsOfSqs = SumsOfSqs,
                        R2 = SumsOfSqs/SumsOfSqs[length(SumsOfSqs)],
    ...
    if ( by=="terms" & nterms>=2 & (!is.null(num_orders)) & (!is.null(order_list))){
      ...
      for(ind_orders_tab in 1:num_orders){
        ...
        P.ad <- (rowSums(t(F.Mod.perm[,c(orders[1]+1,(2*nterms+1:(nterms-1)))])>=as.vector(F.Mod.ad))+1)/(permutations+1)
